...
/*
 도로 네트워크가 N-1개의 도로로 이루어짐,
 각 도시의 쌍에는 유일한 경로가 있음
 => 트리 구조
 
 내생각: 최솟값은 lca, 최댓값은 union find라고 생각했음
 이게 아님;
 왜냐면 연결하는 경로 상에서 가장짧은 도로의 길이, 가장 긴 도로의 길이를 구하는 문제이기때문
 경로의 길이가 아니라 도로의 길이이기 때문에..
 
 일반 lca처럼 깊이 저장할 배열,
 노드간의 연결 저장할 배열,
 각 노드의 i번째 부모까지 최댓값과 최솟값을 저장할 배열,
 (업데이트를 한꺼번에 시킬 수 있고 따로따로 시킬 수 있음,
 2^i번째 부모들만 저장함)
 dp 배열에 대해 minimum, maximum저장 (현재까지의 min과 max값 비교해야 함
 
 결국 dp배열을 저장할 때(부모 저장할 때) min과 max값을 같이 저장하며 값을 업데이트
 
 lca를 잘 저장해주고 그때그때 파악해줘야함
 */
#include <iostream>
#include <vector>
#include <algorithm>
